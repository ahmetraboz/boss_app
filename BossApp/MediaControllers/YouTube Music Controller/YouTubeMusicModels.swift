...
struct YouTubeMusicConfiguration: Sendable {
    ...
    private static func decoded(_ scalars: [UInt8]) -> String {
        String(decoding: scalars, as: UTF8.self)
    }

    private static var localBridgeURL: String {
        var components = URLComponents()
        components.scheme = decoded([104, 116, 116, 112])
        components.host = decoded([108, 111, 99, 97, 108, 104, 111, 115, 116])
        components.port = 26538
        return components.string ?? "/"
    }

    private static var bridgeBundleIdentifier: String {
        [
            decoded([99, 111, 109]),
            decoded([103, 105, 116, 104, 117, 98]),
            decoded([116, 104, 45, 99, 104]),
            decoded([121, 111, 117, 116, 117, 98, 101, 45, 109, 117, 115, 105, 99])
        ].joined(separator: ".")
    }
    
    static let `default` = YouTubeMusicConfiguration(
        baseURL: localBridgeURL,

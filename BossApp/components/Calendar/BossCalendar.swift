        ...
        events.filter { event in
            if event.type.isReminder {
                if case .reminder(let completed) = event.type {
                    return !completed || !BossConfig[.hideCompletedReminders]
                }
            }
            // Filter out all-day events if setting is enabled
            if event.isAllDay && BossConfig[.hideAllDayEvents] {
                return false
            }
            return true
        }
...
#Preview {
    CalendarView()
        ...
        .background(.black)
        .environmentObject(BossViewModel())
}

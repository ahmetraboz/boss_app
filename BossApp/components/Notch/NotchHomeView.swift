...
struct MusicControlsView: View {
    ...
    private func songInfo(width: CGFloat) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            MarqueeText(
                ...
                font: .headline,
                nsFont: .headline,
                textColor: BossConfig[.playerColorTinting]
    ...
    private var activeSlots: [MusicControlButton] {
        ...
        let shouldHideEdges = BossConfig[.showCalendar] && BossConfig[.showMirror] && webcamManager.cameraAvailable && vm.isCameraExpanded
...
struct NotchHomeView: View {
    ...
    private var shouldShowCamera: Bool {
        BossConfig[.showMirror] && webcamManager.cameraAvailable && vm.isCameraExpanded
    }

    private var mainContent: some View {
        HStack(alignment: .top, spacing: (shouldShowCamera && BossConfig[.showCalendar]) ? 10 : 15) {
            MusicPlayerView(albumArtNamespace: albumArtNamespace)

            if BossConfig[.showCalendar] {
                CalendarView()
    ...
    var body: some View {
        VStack {
            CustomSlider(
                value: $sliderValue,
                range: 0...duration,
                color: BossConfig[.sliderColor] == SliderColorEnum.albumArt
                    ? Color(nsColor: color).ensureMinimumBrightness(factor: 0.8)
                    : BossConfig[.sliderColor] == SliderColorEnum.accent ? .effectiveAccent : .white,
                dragging: $dragging,
                lastDragged: $lastDragged,

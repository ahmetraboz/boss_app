...
class WebcamManager: NSObject, ObservableObject {
    ...
    private let sessionQueue = DispatchQueue(label: "BossApp.WebcamManager.SessionQueue", qos: .userInitiated)
        ...
        sessionQueue.async { [weak self] in
            ...
            do {
                ...
                guard session.canAddInput(videoInput) else {
                    throw NSError(domain: "BossApp.WebcamManager", code: -1, userInfo: [NSLocalizedDescriptionKey: "Cannot add video input"])
                }

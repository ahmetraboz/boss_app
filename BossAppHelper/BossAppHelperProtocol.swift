...
/*
 To use the service from an application or other process, use NSXPCConnection to establish a connection to the service by doing something like this:
     ...
     connectionToService = NSXPCConnection(serviceName: "com.ahmetboz.bossapp.helper")
     connectionToService.remoteObjectInterface = NSXPCInterface(with: (any BossAppHelperProtocol).self)
     connectionToService.resume()

 Once you have a connection to the service, you can use it like this:

     if let proxy = connectionToService.remoteObjectProxy as? BossAppHelperProtocol {

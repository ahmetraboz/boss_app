...
public enum BossAutostart {
	private static let logger = Logger(subsystem: "com.bossapp.BossAutostart", category: "main")
...
extension BossAutostart {
	final class Observable: ObservableObject {
		var isEnabled: Bool {
			get { BossAutostart.isEnabled }
			set {
				BossAutostart.isEnabled = newValue
			}
		}
	}
}
...
extension BossAutostart {
	/**
	This package comes with a `BossAutostart.Toggle` view which is like the built-in `Toggle` but with a predefined binding and label. Clicking the view toggles “launch at login” for your app.

	```
	struct ContentView: View {
		var body: some View {
			BossAutostart.Toggle()
		}
	}
	```
	...
	*/
	public struct Toggle<Label: View>: View {
		@ObservedObject private var launchAtLogin = BossAutostart.observable
...
extension BossAutostart.Toggle<Text> {
	/**

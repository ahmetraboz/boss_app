...
extension BossConfig.CodableBridge {
...
/**
Any `Value` that conforms to `Codable` and `BossConfig.Serializable` will use `CodableBridge` to do the serialization and deserialization.
*/
...
/**
`RawRepresentableCodableBridge` is needed because, for example, with `enum SomeEnum: String, Codable, BossConfig.Serializable`, the compiler will be confused between `RawRepresentableBridge` and `TopLevelCodableBridge`.
*/
extension BossConfig {
...
/**
We need both `SetBridge` and `SetAlgebraBridge` because `Set` conforms to `Sequence` but `SetAlgebra` does not. `Set` conforms to `Sequence`, so we can convert it into an array with `Array.init<S>(S)` and store it in the `UserDefaults`. But `SetAlgebra` does not, so it is hard to convert it into an array. Thats why we need the `BossConfig.SetAlgebraSerializable` protocol to convert it into an array.
*/
extension BossConfig {
...
extension BossConfig {
	public struct AnyBridge: Bridge, Sendable {
		public typealias Value = BossConfig.AnySerializable
		public typealias Serializable = Any

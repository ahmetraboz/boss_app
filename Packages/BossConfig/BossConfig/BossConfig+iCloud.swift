...
extension BossConfig {
	...
	To synchronize a key with iCloud, set `iCloud: true` on the ``BossConfig/Key``. That's it! ✨
	...
	- Important: Enable the `iCloud` capability for the app target and turn on “Key-value storage”.

	## Notes

	- If there is a conflict, it will use the latest change.
	- Max 1024 keys and a total of 1 MB storage.
	- It uses `NSUbiquitousKeyValueStore` internally.

	## Dynamically Toggle Syncing

	You can also toggle the syncing behavior dynamically using the ``BossConfig/iCloud/add(_:)`` and ``BossConfig/iCloud/remove(_:)-3074m`` methods.
	...
	if shouldSync {
		BossConfig.iCloud.add(.isUnicornMode)
	}
	```
	*/
	public enum iCloud {
		/**
		The singleton for BossConfig's iCloudSynchronizer.
		*/
		...
		The synced keys.
		*/
		public static var keys: Set<BossConfig.Keys> { synchronizer.keys }
		...
		*/
		// TODO: Support array of BossConfig.Key after Swift 6 pack iteration is supported.
		// Variadic support can be revisited when language support for this target set improves.
		public static func add<each Value: BossConfig.Serializable>(_ keys: repeat BossConfig.Key<each Value>) {
		...
		*/
		public static func remove<each Value>(_ keys: repeat BossConfig.Key<each Value>) {
		...
		Remove the keys that are set to be automatically synced.
		*/
		public static func remove(_ keys: [BossConfig._AnyKey]) {
		...
		*/
		...
		Task {
			...
			print(BossConfig[.isUnicornMode])
			//=> true

			await BossConfig.iCloud.waitForSyncCompletion()
		...
		*/
		...
		// These can be promoted if a concrete product requirement appears.

		/**
		Create synchronization tasks for all the keys that have been added to the ``BossConfig/iCloud``.
		*/
		...
		- Note: `source` should be specified if `key` has not been added to ``BossConfig/iCloud``.
		*/
		static func syncWithoutWaiting(_ keys: BossConfig.Keys..., source: DataSource? = nil) {
...
final class iCloudSynchronizer {
	...
	*/
	private let remoteStorage: any BossConfigKeyValueStore
	...
	*/
	@_BossConfigAtomic(value: []) private(set) var keys: Set<BossConfig.Keys>

	/**
	A thread-safe synchronization status monitor for `keys`.
	*/
	@_BossConfigAtomic(value: []) private var remoteSyncingKeys: Set<BossConfig.Keys>

	// TODO: Replace it with async stream when Swift supports custom executors.
	private lazy var localKeysMonitor: BossConfig.CompositeBossConfigObservation = .init { [weak self] pair, _ in
		guard
			let self,
	...
	*/
	func add<Value: BossConfig.Serializable>(_ key: BossConfig.Key<Value>) {
	...
	*/
	func remove<each Value>(_ keys: repeat BossConfig.Key<each Value>) {
	...
	Remove the keys and stop the observation.
	*/
	func remove(_ keys: [BossConfig._AnyKey]) {
	...
	*/
	...
	- Parameter keys: If the keys parameter is an empty array, the method will use the keys that were added to `BossConfig.iCloud`.
	- Parameter source: Sync keys from which data source (remote or local).

	- Note: The synchronization task might be aborted if both the remote and local data sources do not exist.
	*/
	func syncWithoutWaiting(_ keys: [BossConfig.Keys] = [], _ source: BossConfig.iCloud.DataSource? = nil) {
	...
	*/
	private func syncKey(_ key: BossConfig.Keys, source: BossConfig.iCloud.DataSource) async {
	...
	*/
	private func syncFromRemote(forKey key: BossConfig.Keys) async {
	...
	*/
	private func syncFromLocal(forKey key: BossConfig.Keys) {
		guard
	...
	*/
	private func syncRemoteStorageOnChange() {
		if BossConfig.iCloud.syncOnChange {
			synchronize()
		}
	}
	...
	*/
	private func timestamp(forKey key: BossConfig.Keys, source: BossConfig.iCloud.DataSource) -> Date? {
		switch source {
		case .remote:
			guard
	...
	*/
	func recordTimestamp(forKey key: BossConfig.Keys, timestamp: Date?, source: BossConfig.iCloud.DataSource) {
		switch source {
		case .remote:
			guard
				let values = remoteStorage.object(forKey: key.name) as? [Any],
	...
	*/
	private func latestDataSource(forKey key: BossConfig.Keys) -> BossConfig.iCloud.DataSource? {
...
extension iCloudSynchronizer {
	...
	private static func logKeySyncStatus(
		_ key: BossConfig.Keys,
		source: BossConfig.iCloud.DataSource?,
		syncStatus: SyncStatus,
		value: Any? = nil
	) {
		guard BossConfig.iCloud.isDebug else {
			return
		}
	...
	private static func log(_ message: String) {
		...
		logger.debug("[BossConfig.iCloud] \(message)")
	}
}

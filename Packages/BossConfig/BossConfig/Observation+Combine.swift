...
extension BossConfig {
	...
	*/
		final class BossConfigSubscription<SubscriberType: Subscriber>: Subscription where SubscriberType.Input == BaseChange {
		private var subscriber: SubscriberType?
			private var observation: BossConfigObservationWithLifetime?
		...
		init(subscriber: SubscriberType, suite: UserDefaults, key: String, options: ObservationOptions) {
			...
			self.options = options
				self.observation = BossConfigObservationWithLifetime(
				object: suite,
				key: key,
				observationCallback
			)
	...
	/**
		Custom Publisher, which is using BossConfigSubscription.
		*/
		struct BossConfigPublisher: Publisher {
		...
		func receive(subscriber: some Subscriber<Output, Failure>) {
			let subscription = BossConfigSubscription(
	...
	let publisher = BossConfig.publisher(.isUnicornMode).map(\.newValue)
	...
	*/
	public static func publisher<Value: Serializable>(
		...
		options: ObservationOptions = [.initial]
			...
			let publisher = BossConfigPublisher(suite: key.suite, key: key.name, options: options)
	...
	*/
	public static func publisher(
		...
		return
			keys
				.map { key in
					BossConfigPublisher(suite: key.suite, key: key.name, options: options)
						.map { _ in () }
						.eraseToAnyPublisher()
				}

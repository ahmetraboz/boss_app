...
public protocol _BossConfigObservation: AnyObject {
	func invalidate()
	...
	```swift
	BossConfig.observe(.xyz) { [unowned self] change in
...
extension BossConfig {
	public typealias Observation = _BossConfigObservation
	...
	Any `BossConfig` key changes made within the closure will not propagate to `BossConfig` event listeners (`BossConfig.observe()` and `BossConfig.publisher()`). This can be useful to prevent infinite recursion when you want to change a key in the callback listening to changes for the same key.

	- Note: This only works with `BossConfig.observe()` and `BossConfig.publisher()`. User-made KVO will not be affected.

	```swift
	let observer = BossConfig.observe(keys: .key1, .key2) {
		// …

		BossConfig.withoutPropagation {
			// Update `.key1` without propagating the change to listeners.
			BossConfig[.key1] = 11
		}

		// This will be propagated.
		BossConfig[.someKey] = true
	}
	```
	*/
	...
	/**
	Standard observation for `BossConfig`.
	The only class which handle the low level observation.
	*/
	final class BossConfigObservation: NSObject {
		...
		init(key: BossConfig._AnyKey, _ callback: @escaping Callback) {
	...
	/**
	Observation that wraps `BossConfigObservation` and adds a lifetime association.
	*/
	final class BossConfigObservationWithLifetime: Observation {
		private var observation: BossConfigObservation
		private var lifetimeAssociation: LifetimeAssociation?

		init(object: UserDefaults, key: String, _ callback: @escaping BossConfigObservation.Callback) {
			self.observation = .init(object: object, key: key, callback)
		}

		init(key: BossConfig._AnyKey, _ callback: @escaping BossConfigObservation.Callback) {
	...
	/**
	Observation that manages multiple `BossConfigObservation`.
	Can add or remove the observed key dynamically.
	*/
	final class CompositeBossConfigObservation: Observation {
		private var observations: Set<BossConfigObservation> = []
		private let callback: BossConfigObservation.Callback
		private var lifetimeAssociation: LifetimeAssociation?

		init(_ callback: @escaping BossConfigObservation.Callback) {
			self.callback = callback
		}
		...
		func add(key: BossConfig._AnyKey, options: ObservationOptions = []) {
			let (isInserted, observation) = observations.insert(BossConfigObservation(key: key, callback))
		...
		func remove(key: BossConfig._AnyKey) {
			guard let observation = observations.remove(BossConfigObservation(key: key, callback)) else {
				return
			}
	...
	let observer = BossConfig.observe(.isUnicornMode) { change in
		print(change.newValue)
		//=> false
	}
	```

	- Warning: This method exists for backwards compatibility and will be deprecated sometime in the future. Use ``BossConfig/updates(_:initial:)-88orv`` instead.
	*/
	public static func observe<Value: Serializable>(
		...
		let observation = BossConfigObservationWithLifetime(key: key) { _, change in
			handler(
	...
	let observer = BossConfig.observe(keys: .setting1, .setting2) {
		// …
	}
	```
	...
	*/
	public static func observe(
		...
		let compositeObservation = CompositeBossConfigObservation { _, _ in
			handler()
		}
...
extension BossConfig.ObservationOptions {
...
extension BossConfig.KeyChange: Equatable where Value: Equatable {}
extension BossConfig.KeyChange: Sendable where Value: Sendable {}

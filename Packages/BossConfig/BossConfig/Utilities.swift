...
*/
public protocol _BossConfigOptionalProtocol: ExpressibleByNilLiteral {
	/**
...
extension Optional: _BossConfigOptionalProtocol {
...
extension Sequence {
	...
	*/
	func compact<T>() -> [T] where Element == T? {
		// TODO: Make this `compactMap(\.self)` when the related compiler issue is fixed.
		compactMap { $0 }
	}
}
...
extension BossConfig.Serializable {
	...
	```
	*/
	static func toValue<T: BossConfig.Serializable>(_ anyObject: Any, type: T.Type = Self.self) -> T? {
		if
		...
		guard
			let nextType = T.Serializable.self as? any BossConfig.Serializable.Type,
			nextType != T.self
		else {
			// This is a special case for the types which do not conform to `BossConfig.Serializable` (for example, `Any`).
	...
	*/
	@usableFromInline
	static func toSerializable<T: BossConfig.Serializable>(_ value: T) -> Any? {
		...
		guard let next = serialized as? any BossConfig.Serializable else {
			// This is a special case for the types which do not conform to `BossConfig.Serializable` (for example, `Any`).
			return serialized
		}
...
class Lock: BossConfigLockProtocol {
...
// TODO: Replace with native atomics when the language support is stable enough for this target set.
@propertyWrapper
final class _BossConfigAtomic<Value> {
	private let lock: Lock = .make()
...
#if DEBUG
/**
Get the SwiftUI dynamic shared object.
*/
@usableFromInline
...
func runtimeWarn(
	...
	if !condition {
		os_log(
			...
			log: OSLog(subsystem: "com.apple.runtime-issues", category: "BossConfig"),
			"%@",
			message
		)
	}
